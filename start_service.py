...
if __name__ == '__main__':
    ...
    ssh_url = r['ssh_url']
    owner = r['owner']['login']
    os.system(f'''cd {service_name} && rm -rf .git && git init && git remote add origin {http_url} && git add . && git commit -m "Initial commit" && git push origin master''')
    print(f'git repository: {ssh_url}')
    print(f'git http address: {http_url}')
    ...
    while not ssh_key:
        ...
        time.sleep(3)
    print(f'server ssh key: {ssh_key}')
